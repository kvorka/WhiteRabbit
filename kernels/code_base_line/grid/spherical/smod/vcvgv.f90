submodule (SphericalHarmonics) vcvgv
  ...
  subroutine init_vcvgv_sub(this)
    ...
    allocate(testField3(12,step,this%nFourier))
      this%fftw_12_forw = fftw_plan_many_dft( 1, (/this%nFourier/), 12*step, testField3, iemb, 12*step, 1,                &
                                            &                                testField3, oemb, 12*step, 1, +1, fftw_flags )
    ...
    write(*,*) 'vcvgv initialized'

  end subroutine init_vcvgv_sub

  function vcvgv_fn(this, cajml, cbjml) result(cjml)
    class(T_lateralGrid), intent(in) :: this
    complex(kind=dbl),    intent(in) :: cajml(:), cbjml(:)
    complex(kind=dbl)                :: cjml(this%jmv)
    integer                          :: i, k, j, m, l, jm_int, mj_int, lm_int, jml_int, i1, i2, mj
    real(kind=dbl),     allocatable  :: gc(:), pmm(:), pmj(:), pmj1(:), pmj2(:), cosx(:), fftLege(:), fft(:,:,:)
    complex(kind=dbl),  allocatable  :: sum1(:), sum2(:), sum3(:), cab(:,:), cc(:,:), cr(:,:)
    complex(kind=dbl),  allocatable  :: sumLegendreN(:,:,:), sumLegendreS(:,:,:), fftNC(:,:,:), fftSC(:,:,:)

    allocate(cab(4,this%jmv1), sum1(2), sum2(2), sum3(2), gc(2)) ; cab = cmplx(0._dbl, 0._dbl, kind=dbl)
      
      do jml_int = 1, this%jmv
        cab(1,jml_int) = cajml(jml_int)
      end do
        ...
        do m = 0, j
          ...
          if (m == 0) then
            do l = abs(j-1), min(this%jmax, j+1)
              sum1 = sum1 + conjg( cbjml(3*(l*(l+1)/2+m+1)+j-l) ) * cleb1_fn(j,m,1,-1,l,m-1) * gc * (-1)**(j+l)
              sum3 = sum3 +        cbjml(3*(l*(l+1)/2+m+0)+j-l)   * cleb1_fn(j,m,1, 0,l,m+0) * gc
              sum2 = sum2 +        cbjml(3*(l*(l+1)/2+m+1)+j-l)   * cleb1_fn(j,m,1,+1,l,m+1) * gc
            end do
          else
            do l = max(abs(m-1), j-1), min(this%jmax, j+1)
                           sum1 = sum1 + cbjml(3*(l*(l+1)/2+m-1)+j-l) * cleb1_fn(j,m,1,-1,l,m-1) * gc
              if (l > m-1) sum3 = sum3 + cbjml(3*(l*(l+1)/2+m+0)+j-l) * cleb1_fn(j,m,1, 0,l,m+0) * gc
              if (l > m+0) sum2 = sum2 + cbjml(3*(l*(l+1)/2+m+1)+j-l) * cleb1_fn(j,m,1,+1,l,m+1) * gc
            end do
          end if
    ...
    allocate(cc(12, this%jms2), sum1(4), sum2(4), sum3(4))
      ...
      cc(:,1:this%maxj+1) = cc(:,1:this%maxj+1) / 2 ; cc(3,:) = 2 * cc(3,:)

    deallocate(cab, sum1, sum2, sum3)
    allocate( pmm(step), pmj(step), pmj1(step), pmj2(step), cosx(step), fftLege(step), sumLegendreN(12,step,0:this%nFourier-1), &
            & sumLegendreS(12,step,0:this%nFourier-1), fft(step,3,0:this%nFourier-1), fftNC(step,3,0:this%nFourier/2),          &
            & fftSC(step,3,0:this%nFourier/2), cr(3,this%jms2) ) ; cr = cmplx(0._dbl, 0._dbl, kind=dbl)
      ...
      do i = 1, this%nLegendre, step
        ...
        call fftw_execute_dft(this%fftw_12_forw, sumLegendreN, sumLegendreN)
        ...
        call fftw_execute_dft(this%fftw_12_forw, sumLegendreS, sumLegendreS)
        do concurrent (i1 = 0:this%nFourier-1, i2 = 1:step)
    ...
    deallocate( cc, fft, fftNC, fftSC, sumLegendreN, sumLegendreS, pmm, pmj, pmj1, pmj2, cosx, fftLege )
  ...
  subroutine deallocate_fftw_vcvgv_sub(this)
    class(T_lateralGrid), intent(inout) :: this

    call fftw_destroy_plan( this%fftw_12_forw )
