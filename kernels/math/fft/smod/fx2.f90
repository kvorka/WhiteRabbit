submodule (Fourier_transform) fx2
  ...
  module pure subroutine fxzm2a(m, k, l, x, t)
    ...
    do concurrent ( j = 0:k-1, i = 1:l/2 , iv = 1:m )
      x1re = x(iv,1,i,0,j) - t(1,j) * x(iv,1,i,1,j)
      x1im = x(iv,2,i,0,j) - t(2,j) * x(iv,1,i,1,j)
      
      x(iv,1,i,1,j) =     x1re          + t(2,j) * x(iv,2,i,1,j)
      x(iv,2,i,1,j) =     x1im          - t(1,j) * x(iv,2,i,1,j)
