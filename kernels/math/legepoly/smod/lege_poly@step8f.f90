submodule (lege_poly) step8f
  ...
  module pure subroutine forward_legesum_8_sub(this, nf, roots, weights, sumN, sumS, cr)
    class(T_legep),    intent(in)    :: this
    integer,           intent(in)    :: nf
    real(kind=dbl),    intent(in)    :: roots(8), weights(8)
    ...
    allocate( pmj2(8), pmj1(8), pmj0(8), pmm(8), csx(8), snx(8), ssm(8*nf), asm(8*nf) )
    ...
    do m = 0, this%jmax
      call this%forward_rcb_8_sub( nf, weights(1), sumN(8*nf*m+1), sumS(8*nf*m+1), ssm(1), asm(1) )
