submodule (SphericalHarmonics) vcsv_vcvgv
  ...
  subroutine vcsv_vcvgv_sub(this, ri, dv_r, v, cjm)
    ...
    allocate( pmm(step), pmj(step), pmj1(step), pmj2(step), cosx(step), fftLege(step), sinx(step), symL(step,16), asymL(step,16), &
            & sumLegendreN(16,step,0:this%maxj), sumLegendreS(16,step,0:this%maxj), grid(16,step,0:this%nFourier-1),  &
            & cr(3,this%jms1), fft(3,step,0:this%nFourier-1), sumFourierN(3,step,0:this%jmax+1), symF(step,3), asymF(step,3), &
            & sumFourierS(3,step,0:this%jmax+1) )
      ...
      do i = 1, this%nLegendre, step
        ...
        m = 0
          ...
          j = m
            ...
            do concurrent ( i1=1:16 , i2=1:step )
