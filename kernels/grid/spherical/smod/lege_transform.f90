submodule (SphericalHarmonics) lege_transform
  ...
  pure subroutine pmj_backward_sub(nb, legep, cc, legesum)
    ...
    do concurrent ( i1 = 1:nb )
      legesum(1,i1) = legesum(1,i1) + cc(i1) * legep(1)
      legesum(2,i1) = legesum(2,i1) + cc(i1) * legep(2)
      legesum(3,i1) = legesum(3,i1) + cc(i1) * legep(3)
      legesum(4,i1) = legesum(4,i1) + cc(i1) * legep(4)
    end do
  ...
  pure subroutine pmj_forward_sub(nf, legep, legesum, cr)
    ...
    do concurrent ( i1=1:nf )
      cr(i1) = cr(i1) + legep(1) * legesum(1,i1) + &
                      & legep(2) * legesum(2,i1) + &
                      & legep(3) * legesum(3,i1) + &
                      & legep(4) * legesum(4,i1)
    end do
  ...
  module pure subroutine lege_transform_sub(this, nf, nb, cc, cr, grid_sub)
    ...
    allocate( pmm(4), pmj0(4), pmj1(4), pmj2(4), csx(4), snx(4), ssm(4,nb), asm(4,nb), &
    ...
    do i = 1, this%nLegendre, 4
      ...
      do m = 0, this%jmax2
        ...
        do concurrent ( i2=1:4, i1=1:nb )
          psumN(i1,i2,m) = ssm(i2,i1) + asm(i2,i1)
          psumS(i1,i2,m) = ssm(i2,i1) - asm(i2,i1)
        end do
      end do
      ...
      do concurrent ( m = 0:this%jmax2, i2 = 1:4, i1 = 1:nf )
        psumN(i1,i2,m) = this%weight(i+i2-1) * psumN(i1,i2,m)
        psumS(i1,i2,m) = this%weight(i+i2-1) * psumS(i1,i2,m)
      end do
      
      do m = 0, this%jmax2
        do concurrent ( i1=1:nf, i2=1:4 )
          ssm(i2,i1) = psumN(i1,i2,m) + psumS(i1,i2,m)
          asm(i2,i1) = psumN(i1,i2,m) - psumS(i1,i2,m)
        end do
        ...
        !j = m
          ...
          call pmj_mmset_sub( m, this%cmm(m), snx(1), pmm(1), pmj2(1), pmj1(1), pmj0(1) )
          call pmj_forward_sub( nf, pmj0(1), ssm(1,1), cr(1,mj) )
        
        do j = 1, (this%jmax2-m)/2
          mj = mj+2
          
          call pmj_recursion_sub( this%amj(mj-1), this%bmj(mj-1), csx(1), pmj2(1), pmj1(1), pmj0(1) )
          call pmj_forward_sub( nf, pmj0(1), asm(1,1), cr(1,mj-1) )
