submodule (SphericalHarmonics) vcvv_vcvgv
  ...
  module pure subroutine vcvv_vcvgv_sub(this, ri, q, dv_r, v, cjm)
    ...
    do j = 0, this%jmax+1
      ...
      do m = 0, j
        ...
        ijml = 3*(j*(j+1)/2+m)+abs(j-1)-j
          cab(3,ijml) =         ( sum1(1) - sum2(1) ) * sq2_1 * fac1
          cab(4,ijml) = cunit * ( sum1(1) + sum2(1) ) * sq2_1 * fac1
          cab(5,ijml) =         ( sum3(1)           )         * fac1
        
        ijml = 3*(j*(j+1)/2+m)+(j+1)-j
          cab(3,ijml) =         ( sum1(2) - sum2(2) ) * sq2_1 * fac2
          cab(4,ijml) = cunit * ( sum1(2) + sum2(2) ) * sq2_1 * fac2
    ...
    do concurrent ( mj = 1:this%jms1 )
      cr12     = +( cr(2,mj) - cr(3,mj) * cunit ) * sq2_1
      cr(3,mj) = -( cr(2,mj) + cr(3,mj) * cunit ) * sq2_1
      cr(2,mj) = cr12
    end do
    ...
    do concurrent ( ijm=1:this%jms , i1=1:4 )
